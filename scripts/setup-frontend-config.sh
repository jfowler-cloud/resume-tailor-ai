...
echo -e "${BLUE}🔧 Setting up frontend configuration from deployed stack...${NC}"

# Get the stack name (default: ResumeTailorStack)
STACK_NAME="${1:-ResumeTailorStack}"
REGION="${AWS_REGION:-us-east-1}"

echo -e "${BLUE}📡 Fetching outputs from CloudFormation stack: ${STACK_NAME}${NC}"
...
if [ -z "$OUTPUTS" ] || [ "$OUTPUTS" == "null" ]; then
  echo -e "${RED}❌ Failed to fetch stack outputs. Is the stack deployed?${NC}"
  exit 1
fi
...
if [ -z "$USER_POOL_ID" ] || [ -z "$USER_POOL_CLIENT_ID" ] || [ -z "$IDENTITY_POOL_ID" ] || [ -z "$BUCKET_NAME" ] || [ -z "$STATE_MACHINE_ARN" ]; then
  echo -e "${RED}❌ Missing required outputs from stack${NC}"
  exit 1
fi

# Create frontend .env file
ENV_FILE="frontend/.env"
echo -e "${BLUE}📝 Writing configuration to ${ENV_FILE}${NC}"
...
echo -e "${BLUE}📝 Writing deployment config to ${DEPLOY_CONFIG}${NC}"
...
echo -e "${GREEN}✅ Deployment config created${NC}"
...
echo "  • Deploy to CloudFront: ./deploy-frontend.sh"
